<template>
  <div class="yt-pull" :style="{ height: wrapperHeight, transform: `translate3d(0, ${diff}px, 0)` }">
    <div v-if="topLoadMethod" class="yt-pull-action"
         :style="{ height: `${topBlockHeight}px`, marginTop: `${-topBlockHeight}px` }">
      <!--@slot 自定义下拉刷新的内容-->
      <slot name="top-block" :state="state" :state-text="topText">
        <p class="yt-pull-text"><img :class="{'is-down': state === 'pull', 'is-up': state === 'trigger'}"
                                     class="yt-pull-icon" :src="stateSrc">{{ topText }}</p>
      </slot>
    </div>
    ...
    <div v-if="bottomLoadMethod" class="yt-pull-action"
      ...
      <slot name="bottom-block" :state="state" :state-text="bottomText">
        <p class="yt-pull-text"><img :class="{'is-down': state === 'trigger', 'is-up': state === 'pull'}"
                                     class="yt-pull-icon" :src="stateSrc">{{ bottomText }}</p>
      </slot>
    </div>
  </div>
</template>
